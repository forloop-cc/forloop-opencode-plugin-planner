...
**全部基于 Loop Engineering 构建**——一个持续反馈循环：Plan（规划）→ Code（编码）→ Review（审查）→ Deploy（部署）→ Learn（学习）。每个循环都会生成数据反馈到下一个循环，使您的团队在每次迭代中越来越快。此插件驱动 **Plan（规划）** 阶段，为您的代理提供直接访问 Sprint、用户故事和任务跟踪的能力。
